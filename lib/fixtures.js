...
const weekOne = moment('04-02-20', 'DD-MM-YY');
...
exports.showFixturesNoButtons = (input) => {
	...
	for (let weekNum in groupedInput) {
		...
		const week = {
			...
			fixtures,
			thisWeek: (
				(moment().isBefore(moment(weekOne).add(weekWeek, 'week')) && parseInt(weekNum, 10) === 1)
				|| moment().isBetween(moment(weekOne).add(weekWeek, 'week'), moment(weekOne).add(weekWeek + 1, 'week'))
			)
		};
		weeks.push(week);
	}
