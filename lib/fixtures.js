...
const weekOne = moment('18-10-19', 'DD-MM-YY');
...
exports.makeFixtures = (input) => {
	...
	while (offset < playerCount) {
		...
		let weekString = moment(weekOne).add(offset, 'week').format('DD/MM');
		weekString += ' - ';
		weekString += moment(weekOne).add(offset, 'week').add(3, 'days').format('DD/MM');
		weeks.push({
			week: offset,
			weekString,
			fixtures,
...
exports.showFixtures = (input) => {
	...
	for (let weekNum in groupedInput) {
		...
		let weekString = moment(weekOne).add(weekNum, 'week').format('DD/MM');
		weekString += ' - ';
		weekString += moment(weekOne).add(weekNum, 'week').add(3, 'days').format('DD/MM');
		const week = {
			week: weekNum,
			weekString,
			fixtures,
